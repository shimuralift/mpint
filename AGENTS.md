...
source and header, and start their own project with them, possibly code agent assisted, or  
develop their own implementation, using an existing one as a starting point in which case  
the existing tests and profiling might be helpful.

In subdirectory mpint/demo, there are eight executable artifacts for  
...
In subdirectory *mpint/test* there is a bash script *test.sh* which runs all six demo variants  
