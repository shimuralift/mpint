...
DEMOGMPXX_SRCS := $(SRCDIR)/demoGMPXX.cpp $(SRCDIR)/detGMPXX.cpp
DEMOGMPXX_HDRS := $(SRCDIR)/MPintGMPXX.hpp $(SRCDIR)/det.hpp
TARGETGMPXX   := $(BINDIR)/demoGMPXX


.PHONY: all clean

all: $(TARGET) $(TARGETGMP) $(TARGETGMPXX)
...
$(TARGETGMPXX): $(DEMOGMPXX_SRCS) $(DEMOGMPXX_HDRS)
	$(CXX) $(CXXFLAGS) -o $@ $(DEMOGMPXX_SRCS) -lgmpxx -lgmp


clean:
	$(RM) $(TARGET) $(LIBRARY) $(LIB_OBJ) $(TARGETGMP) $(LIBRARYGMP) $(LIBGMP_OBJ) $(TARGETGMPXX)
